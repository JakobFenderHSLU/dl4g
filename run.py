...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--n_games", default=100, type=int, help="Number of games to play"
    )
    parser.add_argument(
        "-ll",
        "--log_level",
    ...
    logger.info(
        f"Agent: {args.agent_trump_strategy} - {args.agent_play_strategy} - {args.agent_play_rule_strategies}"
    )
    logger.info(
        f"Opponent: {args.opponent_trump_strategy} - {args.opponent_play_strategy} - "
    ...
    def _get_trump_strategy(strategy_name: str) -> TrumpStrategy:
        if strategy_name == "random":
            ...
            return StatisticalTrumpStrategy(
    ...
    def _get_play_rule_strategies(strategies_names: str) -> List[PlayRuleStrategy]:
        ...
        for strategy_name in strategies_names:
            if strategy_name == "only_valid":
                strategies.append(
                    OnlyValidPlayRuleStrategy(seed=args.seed, log_level=args.log_level)
                )
            if strategy_name == "smear":
                strategies.append(
                    SmearPlayRuleStrategy(seed=args.seed, log_level=args.log_level)
                )
