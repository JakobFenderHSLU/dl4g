...
class ResultsUtils:
    ...
    def print_results(self, arenas: List[Arena]):
        ...
        agents_wins = len(
            arenas[0].points_team_0[arenas[0].points_team_0 > arenas[0].points_team_1]
        ) + len(
            arenas[1].points_team_1[arenas[1].points_team_1 > arenas[1].points_team_0]
        )
        ...
        agents_points = np.concatenate(
            (arenas[0].points_team_0, arenas[1].points_team_1)
        )
        opponents_points = np.concatenate(
            (arenas[0].points_team_1, arenas[1].points_team_0)
        )
        ...
        table.add_row(
            ["Winrate", f"{agents_winrate:.2f} %", f"{opponents_winrate:.2f} %"]
        )
        table.add_row(
            [
                "Average Points",
                sum(agents_points) / len(agents_points),
        ...
        table.add_row(
            [
                "Points 25th Percentile",
                np.quantile(agents_points, 0.25),
                np.quantile(opponents_points, 0.25),
            ]
        )
        table.add_row(
            ["Points Median", np.median(agents_points), np.median(opponents_points)]
        )
        table.add_row(
            [
                "Points 75th Percentile",
                np.quantile(agents_points, 0.75),
        ...
        agent_trump_rounds = np.concatenate(
            (arenas[0].points_team_0[1::2], arenas[1].points_team_1[::2])
        )
        agent_trump_rounds_winrate = (
            np.concatenate(
                (
        ...
        opponent_trump_rounds = np.concatenate(
            (arenas[0].points_team_1[::2], arenas[1].points_team_0[1::2])
        )
        opponent_trump_rounds_winrate = (
            np.concatenate(
                (
        ...
        table.add_row(
            [
                "Winrate",
                f"{agent_trump_rounds_winrate:.2f} %",
                f"{opponent_trump_rounds_winrate:.2f} %",
            ]
        )
        table.add_row(
            [
                "Average Points",
                sum(agent_trump_rounds) / len(agent_trump_rounds),
                sum(opponent_trump_rounds) / len(opponent_trump_rounds),
            ]
        )
        table.add_row(
            ["Max Points", max(agent_trump_rounds), max(opponent_trump_rounds)]
        )
        table.add_row(
            ["Min Points", min(agent_trump_rounds), min(opponent_trump_rounds)]
        )
        table.add_row(
            [
                "Points 25th Percentile",
                np.quantile(agent_trump_rounds, 0.25),
        ...
        table.add_row(
            [
                "Points Median",
                np.median(agent_trump_rounds),
                np.median(opponent_trump_rounds),
            ]
        )
        table.add_row(
            [
                "Points 75th Percentile",
                np.quantile(agent_trump_rounds, 0.75),
    ...
    def _print_stacked_bar_plot(self, bar_length, agents_wins, opponents_wins):
        ...
        self._logger.info(f"[{plus}{minus}]")
        self._logger.info("")
